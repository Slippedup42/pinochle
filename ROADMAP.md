...
6. **Correctness net** — **done (#125).** Seeded-scenario parity
   ...
   **The net now covers everything that crosses the boundary.** This one
   covers the rules engine, `export_evaluator.py --check` covers the
   evaluator model, `generate_rollout_dataset.py --check` covers the
   dataset that model is fit to (#225), and since #211
   `export_stats_parity.py --check` covers `web/src/ab/stats.ts` — the
   three A/B statistics, which were the last hand-port with nothing
   behind them and are what every shipped strategy decision was judged
   by. All four fire only from `python -m pytest -q`; see the CI note
   above.
